...
One jar is published per target, but a target accepts more than the one NeoForge build it was compiled
against: 26.1 is built on 26.1.2 and has to run on 26.1 and 26.1.1 as well, and every target's range reaches
back to builds older than the one it compiles against. Compiling the source against each of them is not
enough, because javac binds every call to whatever overload the compile classpath offers, and the jar is only
ever compiled once. This reads every game method, field and class the compiled jar refers to, straight out of
its constant pools so method references count too, and asks javap whether each one exists in the other build,
following supertypes the way the JVM resolves them.
...
def references(classes_dir):
	...
	# In batches, so the command line stays inside what Windows allows however many classes there are.
	out = "".join(subprocess.run(["javap", "-v", "-p"] + files[i:i + 50], capture_output=True, text=True,
								 check=True).stdout for i in range(0, len(files), 50))
	refs, current = {}, None
	for line in out.splitlines():
